...
import {
  BarChart2, Database, Activity,
  CheckCircle2, XCircle, Loader2,
  Layers, Zap, Globe, Brain,
  Search, Shield, GitMerge, Scale, AlertCircle, PenTool,
  TrendingUp
} from 'lucide-react'
...
function StatusBadge({ status }) {
  ...
  if (status === 'ok') return (
    <div className="flex items-center gap-1.5 text-sage text-xs font-medium bg-sage/10 px-2.5 py-1 rounded-full">
      <div className="w-1.5 h-1.5 rounded-full bg-sage animate-pulse" />
      System Online
    </div>
  )
  return (
    <div className="flex items-center gap-1.5 text-rose text-xs font-medium bg-rose/10 px-2.5 py-1 rounded-full">
...
function PipelineDiagram() {
  return (
    <div className="px-3 py-2">
      <p className="text-[10px] font-bold text-dim uppercase tracking-wider mb-3">Agent Pipeline</p>
...
/* ── Target segments (clean labels, no "GenZ" strings) ────────────────────── */
function SegmentsPanel({ segments = [] }) {
  if (!segments.length) return null
  return (
    <div className="px-4 py-4 mt-auto border-t border-border/40">
      <p className="text-[10px] font-bold text-dim uppercase tracking-wider mb-3">Target Segments</p>
      <div className="flex flex-wrap gap-1.5">
        {segments.map(s => (
...
export default function Dashboard() {
  ...
  const segments = health?.segments_available || dataset?.segments || []

  return (
    <div className="flex h-screen overflow-hidden warm-bg font-body">

      {/* ══ Sidebar ═════════════════════════════════ */}
      <aside className="w-64 flex-shrink-0 bg-white/50 backdrop-blur-md border-r border-border flex flex-col overflow-y-auto">
        ...
        <div className="px-5 py-6 flex-shrink-0">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <div className="font-display font-bold text-base text-text leading-none tracking-tight"></div>
              <div className="text-[10px] font-bold text-dim/60 uppercase tracking-widest mt-1">Campaign Intelligence</div>
            </div>
          </div>
          <div className="mt-4">
        ...
        {/* KPIs */}
        {dataset?.kpi_metrics?.length > 0 && (
          <div className="mt-2">
            <Divider />
            <KpiList kpis={dataset.kpi_metrics} />
          </div>
        )}
        ...
        <header className="flex-shrink-0 h-16 border-b border-border/40 bg-white/40 backdrop-blur-md flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-dim font-medium leading-none mt-1.5 flex items-center gap-1.5">
                ...
                Agent LangGraph Analysis Engine
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2.5 px-4 py-2 bg-white/50 border border-border/60 rounded-full shadow-soft">
              ...
              <span className="text-xs font-semibold text-text/70 tracking-tight">
                {status === 'ok' ? 'Pipeline Ready' : status === 'loading' ? 'Connecting…' : 'Offline'}
              </span>
            </div>
          </div>
        </header>
